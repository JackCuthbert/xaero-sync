...
`config.txt` remains local-only, but Xaero assigns the selected automatic-world filename only after play begins. A legacy automatic-world download is therefore stored unchanged in Xaero Sync's connection sidecar. When Xaero creates a dimension's selected file, the client records that target and asks the player to reconnect. On the next configuration phase—before Xaero loads waypoint files—the staged server file is atomically copied to that exact target. The raw staged file is then removed, while the small target mapping remains so a v1 server's legacy filename can be restored without prompting again. No waypoint file is merged or rewritten while Xaero is running.
